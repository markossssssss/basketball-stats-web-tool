...
class BaseHighlightModelFast():
    def __init__(self, game_dir, font_path=None, logo_path=None, video_dir_postfix=""):
        ...
        self.missed_data = event_data[(event_data.Info == "不进") | (event_data.Event == "失误") | (event_data.Event == "抢断") | (event_data.Event == "盖帽")]
    ...
    def get_all_in_one_highlight(self, music_path=None):
        target_path = os.path.join(self.game_dir, "highlight_all.mp4")
    ...
    def get_all_teams_highlights(self, music_path=None):
        for i in range(self.num_teams):
            ...
            self.collections_whole_team[i].download_highlight(self.videos, self.quarter_video_lens, music_path, target_path)
    ...
    def get_all_players_highlights(self, team_id, music_path=None, target_stats=None, add_cover=True):
        ...
        if team_id is not None:
            for name in self.collections_team_players[team_id]:
                ...
                video_path = self.collections_team_players[team_id][name].download_highlight(self.videos, self.quarter_video_lens, music_path, target_path)
                if video_path:
...
class TeamHighLightCollection():
    ...
    def download_highlight(self, videos, quarter_video_lens, music_path=None, result_path=None):
        if result_path is not None:
            print(f'generating: {os.path.basename(result_path)}')
        if not (len(self.data)):
            return 0
        
        clips = []
        for highlight in self.data:
            clips.append(highlight.clip_video(videos, quarter_video_lens))



        final_clip = concatenate_clips(os.path.dirname(videos[0]), result_path, clips)
...
class HighLight():
    ...
    def clip_video(self, videos, quarter_video_lens):
        ...
        return (os.path.basename(videos[self.quarter - 1]), start_time, end_time)
