...
from . import BaseStatsModel, FriendshipMatchStatsModel, terms
...
def name_func(name, team_name):
    pattern = r'(\d+号)'
    name = re.sub(pattern, '', name)
    return f"{team_name}_{name}"


class LeagueStatsModel():
    def __init__(self, game_list, game_dir=None, num_games_to_keep=3, target_dir="history_data_model"):
        ...
        target_stats = ["atpts", "fts_atpts", "2pts_atpts", "3pts_atpts", "mades", "fts_mades", "2pts_mades", "3pts_mades", 
                        "scores", "assists", "rebounds", "steals", "blocks", "blocked",
                    "off_rebounds", "def_rebounds", "fouls", "tos", "make_fouls", "EFF", "oncourt_scores",
                      "oncourt_loses", "rounds", "plus_minus", "oncourt_off_rounds", "oncourt_def_rounds",
                    "oncourt_team_rebounds", "oncourt_opponent_rebounds", "used_rounds", "games_played"]
        self.target_stats = [terms[stat] for stat in target_stats]

        self.parse_stats()
        # self.records = {"得分": 0, "助攻": 0, "篮板": 0, "抢断": 0, "盖帽": 0, "三分数": 0}


    def parse_game_stats(self, game_stats, team_names):
        for i in range(2):
            player_stat = game_stats[i][self.target_stats].astype(int).iloc[:-1]
            team_stat = game_stats[i][self.target_stats].astype(int).iloc[-1:]

            player_stat.reset_index(inplace=True)
            partial_name_func = partial(name_func, team_name=team_names[i])
            player_stat['姓名'] = player_stat['姓名'].apply(partial_name_func)
            player_stat.set_index('姓名', inplace=True)

            team_stat.reset_index(inplace=True)
            team_stat.rename(columns={'姓名': '队名'}, inplace=True)
            team_stat.set_index('队名', inplace=True)

            if self.team_stats is None:
                self.team_stats = team_stat
                self.player_stats = player_stat
            else:
                self.team_stats = pd.concat([self.team_stats, team_stat])
                self.player_stats = pd.concat([self.player_stats, player_stat])

    @staticmethod
    def get_records(player_stats):
        record_df = pd.DataFrame(columns=["队伍", "姓名", "记录项", "记录值"])
        for stat in ["得分", "篮板", "助攻", "抢断", "盖帽", "3分球命中", "失误"]:
            new_row = {"记录项": stat}
            
            record = player_stats.iloc[player_stats[stat].argmax()]
            # print(record)
            new_row["记录值"] = record[stat]
            new_row["队伍"] = record.name.split("_")[0]
            new_row["姓名"] = record.name.split("_")[1]
    ...
    def parse_stats(self):
        ...
        for i, game_name in tqdm(enumerate(self.game_list)):
            # print(game_name)
            ...
            player_model = FriendshipMatchStatsModel(event_data, config)
            player_model.get_stats()

            self.parse_game_stats(player_model.player_stats, player_model.team_names)
            # print(self.team_stats)
            # break
            

        record_df = self.get_records(self.player_stats)
        record_df.to_csv("records.csv")

        self.team_stats = self.team_stats.groupby(self.team_stats.index).sum()
        self.team_stats.to_csv("source_data_team.csv")
        self.player_stats = self.player_stats.groupby(self.player_stats.index).sum()
        self.player_stats.to_csv("source_data_player.csv")
